# Migration Guide: Moving  to Geo Module

## Overview

This guide explains how to migrate from using geographical models in the  module to using the centralized Geo module.
...
Replace imports  models to Geo models:

```diff
- use Modules\<nome modulo>\Models\Region;
- use Modules\<nome modulo>\Models\Province;
- use Modules\<nome modulo>\Models\City;
- use Modules\<nome modulo>\Models\Cap;
...
{
    return $this->belongsTo(\Modules\<nome modulo>\Models\Region::class);
}
...
If you have existing data in the  module's geographical tables, you'll need to migrate it to the Geo module's tables. Create a custom migration for this purpose.
